...
mod test {
    ...
    fn help_intersect_missing_functions() {
        let vec_rs_api = vec![
        ...
        let hash_rs = HashSet::<&str>::from_iter(vec_rs_api);

        let hash_c = HashSet::<&str>::from_iter(vec_c_api);

        println!("Left to implement:\n{:?}", hash_c.difference(&hash_rs));
    }
}
