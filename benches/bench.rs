...
use std::io::Read;
use sfcgal::{SFCGeometry, FromGeoJSON, ToSFCGAL, ToCoordinates, ToGeoJSON, TryInto};
...
fn bench_sfcgal_2d_to_geojson(b: &mut Bencher) {
    let geom = make_sfgal_geom();
    b.iter(||{ let coords = geom.to_geojson::<Point2d>().unwrap(); });
}

#[bench]
fn bench_sfcgal_2d_to_geotype_geometry(b: &mut Bencher) {
    let geom = make_sfgal_geom();
    b.iter(||{ let polyg: geo_types::Geometry<f64> = geom.clone().try_into().unwrap(); });
}

#[bench]
fn bench_sfcgal_2d_to_geotype_concrete_type(b: &mut Bencher) {
    let geom = make_sfgal_geom();
    b.iter(||{ let polyg: geo_types::Polygon<f64> = geom.clone().try_into().unwrap().into_polygon().unwrap(); });
}
